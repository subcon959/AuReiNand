...
void setupCFW(void){
    ...
    const char lastConfigPath[] = "aureinand/lastbootcfg";
    ...
    if(mode) firmPathPatched = emuNAND ? "/aureinand/patched_firmware_emu.bin" :
                                         "/aureinand/patched_firmware_sys.bin";
...
u32 loadEmu(void){
    ...
    const char path[] = "/aureinand/emunand/emunand.bin";
...
u32 patchFirm(void){
    ...
    if(mode){
        ...
        const char path[] = "/aureinand/reboot/reboot.bin";
        u32 size = fileSize(path);
        if(!size) return 0;
